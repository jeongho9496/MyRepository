...
import com.example.administrator.a2cmbeacontest.dto.Sphoto;
    ...
    private void sphotoLoading(final String sid) {
        AsyncTask<Void, Void, List<Sphoto>> asyncTask = new AsyncTask<Void, Void, List<Sphoto>>() {
            ...
            protected void onPostExecute(List<Sphoto> sphotos) {
                ...
                testImage1.setImageBitmap(sphotos.get(0).getSphoto());
                testImage2.setImageBitmap(sphotos.get(1).getSphoto());

                HashMap<String,Bitmap> url_maps = new HashMap<String, Bitmap>();
                for (int i=0; i<sphotos.size(); i++){
                    url_maps.put("photos"+i, sphotos.get(i).getSphoto());
                }
                storeImage.removeAllSliders();

                for (String name : url_maps.keySet()){
                    TextSliderView textSliderView = new TextSliderView(getApplicationContext());
                    textSliderView.image(String.valueOf(url_maps.get(name)));
    ...
    private List<Sphoto> parseJson(String strJson) {
        ...
        try {
            ...
            for (int i=0; i<jsonArray.length(); i++){
                ...
                sphoto.setSphoto(getBitmap(jsonObject.getString("spic_savedfile")));
                list.add(sphoto);
            }
